(() => {
  ...
  function createOverlay() {
    ...
    overlay.querySelector('.lap-capture-preview-meta').innerHTML = `
      <strong>${escapeHtml(pathName(new URL(currentAsset.sourceUrl).pathname) || '网页图片')}</strong>
  ...
  async function saveConfirmedAsset() {
    ...
    if (!response?.ok) {
      ...
      const existing = confirm.querySelector('.lap-capture-inline-error');
      existing?.remove();
      const error = document.createElement('div');
      error.className = 'lap-capture-inline-error';
      error.textContent = response?.error || '保存失败';
