...
const DEFAULT_SYSTEM_PROMPT: &str = r#"You are a visual digital-asset librarian. Analyse the supplied image and return strict JSON only. Use concise, reusable taxonomy tags. Prefer namespace tags such as subject:car, style:minimal, composition:centered, lighting:studio, color:blue, usage:ui-reference. Do not invent project names or people. Return this shape: {\"title\":\"\",\"description\":\"\",\"tags\":[\"namespace:value\"],\"dominantColors\":[\"#RRGGBB\"],\"confidence\":0.0}."#;
...
fn load_config() -> Result<StoredOnlineAiConfig, String> {
    ...
    serde_json::from_slice(&data).map_err(|error| format!("Failed to parse online AI settings: {}", error))
}
...
pub fn save_online_ai_provider(
    ...
    let existing = config.providers.iter().find(|provider| provider.id == provider_id).cloned();
    let api_key = input
        .api_key
        .unwrap_or_default()
        .trim()
    ...
    let base_url = validate_http_url(&input.base_url)?;
    let provider = StoredOnlineAiProvider {
        ...
        min_confidence: input.min_confidence.unwrap_or_else(default_min_confidence).clamp(0.0, 1.0),
        ...
        created_at: existing.as_ref().map(|provider| provider.created_at).unwrap_or(now),
        updated_at: now,
    };

    if let Some(index) = config.providers.iter().position(|item| item.id == provider_id) {
...
pub fn delete_online_ai_provider(provider_id: String) -> Result<bool, String> {
    let mut config = load_config()?;
    let old_len = config.providers.len();
    config.providers.retain(|provider| provider.id != provider_id);
    let changed = config.providers.len() != old_len;
    if changed {
        save_config(&config)?;
    }
    Ok(changed)
...
fn load_image_payload(file_id: i64) -> Result<ImagePayload, String> {
    ...
    let thumbnail = AThumb::fetch(file_id)?
        .and_then(|thumbnail| thumbnail.thumb_data)
        .ok_or_else(|| "该格式没有可用于在线 AI 的预览图".to_string())?;
...
fn analysis_prompt(provider: &StoredOnlineAiProvider) -> String {
    format!(
        "请使用 {} 输出。最多生成 {} 个标签。标签必须稳定、可复用，并优先使用 namespace:value 形式。只输出 JSON，不要 Markdown。",
...
fn apply_custom_headers(
    ...
    if !provider.auth_header.trim().is_empty() {
        let header_name = HeaderName::from_bytes(provider.auth_header.trim().as_bytes())
            .map_err(|error| format!("无效的认证 Header：{}", error))?;
        let value = format!("{}{}", provider.auth_prefix, provider.api_key);
...
async fn call_provider(
    ...
    match provider.kind {
        OnlineAiProviderKind::OpenaiCompatible => {
            let endpoint = join_endpoint(&provider.base_url, "chat/completions");
            ...
            let value = send_json_request(provider, &endpoint, body, &[]).await?;
        ...
        OnlineAiProviderKind::Gemini => {
            ...
            let key_header = provider.api_key.as_str();
            let value = send_json_request(provider, &endpoint, body, &[("x-goog-api-key", key_header)]).await?;
            extract_gemini_text(&value)
        }
        OnlineAiProviderKind::Anthropic => {
            let endpoint = join_endpoint(&provider.base_url, "messages");
            ...
            let value = send_json_request(
                provider,
                &endpoint,
                body,
                &[("x-api-key", provider.api_key.as_str()), ("anthropic-version", "2023-06-01")],
            )
            .await?;
...
fn parse_analysis(raw: &str, provider: &StoredOnlineAiProvider) -> Result<OnlineAiAnalysis, String> {
